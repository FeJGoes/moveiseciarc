    ...
    <tr>
      <td>
        <form id="user-{{$user->id}}-active-form" action="{{route('usuarios.update',['user' => $user->id])}}" method="POST">
          ...
          <input class="uk-checkbox" type="checkbox" name="active" value="1" @if($user->active) checked @endif onchange="document.getElementById('user-{{$user->id}}-active-form').submit()">
        </form>
      </td>
      <td>{{$user->name}}</td>
      <td>{{$user->email}}</td>
